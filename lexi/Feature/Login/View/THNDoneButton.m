...
+ (instancetype)thn_initWithFrame:(CGRect)frame withTitle:(NSString *)title completion:(void (^)(void))completion {
    THNDoneButton *doneButtonView = [[THNDoneButton alloc] initWithFrame:frame];
    doneButtonView.doneHandle = completion;
    [doneButtonView.doneButton setTitle:title forState:(UIControlStateNormal)];
...
- (UIButton *)doneButton {
    if (!_doneButton) {
        _doneButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame), 45)];
...
- (UIImageView *)shadowView {
    if (!_shadowView) {
        _shadowView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 45, CGRectGetWidth(self.frame), 30)];
